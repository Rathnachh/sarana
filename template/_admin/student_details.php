    ...
    ?>
        ...
        <div class="content shadow p-3 bg-white">
            <div class="title d-flex">
                <a href="<?php echo $_GET['link'].'&BDN='.@$_GET['BDN'].'&ROM='.@$_GET['ROM'] ?>"><i class="fa-solid fa-arrow-left mr-3 text-info" style="font-size:18px"></i></a>
                <h3 class="kh-text font-weight-bold text-info">ព័ត៌មានលម្អិតរបស់<?php echo $row['name'] ?></h3>
            </div>
            <div class="row mt-3">
                ...
                <table class="table table-borderless ">
                    ...
                    <tr>
                        <td class="w-25 kh-text">គោត្តនាម / នាម</td>
                        <!-- <td class="kh-text text-info font-weight-bold">: <?php echo $row['fname'] ?> ជាអក្សរឡាតាំង : <?php echo $row['name_en'] ?></td> -->
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['fname'] ?> </td>

                    </tr>
                    <tr>
                        <td class="w-25 kh-text">ជាអក្សរឡាតាំង</td>
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['name_en'] ?></td>
                    </tr>
                    <tr>
                        <td class="w-25 kh-text">កាន់អត្តសញ្ញាណប័ណ្ឌលេខ</td>
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['card_nation_id'] ?></td>
                    </tr>
                    <tr>
                        <td class="w-25 kh-text">ភេទ</td>
                        <td class="kh-text text-info font-weight-bold">: <?php if($row['gender']=='1') echo 'ស្រី';else echo 'ប្រុស'; ?></td>
                    </tr>
                    <tr>
                        <td class="w-25 kh-text">ថ្ងៃខែឆ្នាំកំណើត</td>
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['dob'] ?></td>
                    </tr>
                    <tr>
                        <td class="w-25 kh-text">ជនជាតិ</td>
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['ethnicity'] ?>, សញ្ជាតិ  : <?php echo $row['nation_kh'] ?></td>
                    </tr>
                    <tr>
                        <td class="w-25 kh-text">លេខទូរសព្ទ</td>
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['person_phone'] ?></td>
                    </tr>
                    <tr>
                        <td class="w-25 kh-text">លេខទូរសព្ទអាណាព្យាបាល</td>
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['parent_phone'] ?></td>
                    </tr>
                   
                    <tr>
                        <td class="w-25 kh-text">ទីកន្លែងកំណើត</td>
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['pro_name'].' '.$row['dis_name'].' '.$row['com_name'].' '.$row['village_name'] ?></td>
                    </tr>
                    <tr>
                        <td class="w-25 kh-text">អាសយដ្ឋានបច្ចុប្បន្ន</td>
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['province_kh_name'].' '.$row['district_namekh'].' '.$row['commune_namekh'].' '.$row['village_namekh'] ?></td>
                    </tr>
                </table>
                </div>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <table class="table">
                    <tr>
                        <td class="w-25 kh-text">ស្នាក់នៅ</td>
                        <td class="kh-text text-info font-weight-bold">: 
                            <?php echo $row['name_room'].' '.$row['floor_name'].' '.$row['building_name'] ?>
                            <button type="button" class="btn btn-sm btn-outline-primary kh-text" data-toggle="modal" data-target="#change_room">ប្តូរបន្ទប់</button>
                        </td>
                    </tr>
                   
                    <tr>
                        <td class="w-25 kh-text">វិទ្យាស្ថាន</td>
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['fa_name'] ?></td>
                    </tr>
                    <tr>
                        <td class="w-25 kh-text">ជំនាញ</td>
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['subject_name'] ?></td>
                    </tr>
                    <tr>
                        <td class="w-25 kh-text">សិក្សា</td>
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['year_name'] ?></td>
                    </tr>
                    <tr>
                        <td class="w-25 kh-text">យានយន្ត</td>
                        <td class="kh-text text-info font-weight-bold">: 
                        <?php 
                            if($row['ve_id'] == '')
                            {
                                ?>គ្មានទេ <a href="#" data-toggle="modal" data-target="#create_vihicle" class="kh-text">បន្ថែមយានយន្ត​ ?</a><?php
                            }
                            else 
                            {
                    ...
                    <tr>
                        <td class="w-25 kh-text">ប្រភេទ</td>
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['status_name'] ?></td>
                    </tr>
                    <tr>
                        <td class="w-25 kh-text">ថ្ងៃខែចុះឈ្មោះចូលស្នាក់នៅ</td>
                        <td class="kh-text text-info font-weight-bold">: <?php echo $row['register_date'] ?></td>
                    </tr>
                    </table>
                </div>
            </div>
                ...
                $.ajax({
                    ...
                    success:function(data)
                    {
                        if(data==1)
                        {
                            ...
                            swal('ជោគជ័យ', ' ប្តូរបន្ទប់បានដោយជោគជ័យ '+data,'warning');
                        }
                        setInterval(() => {
                            window.location.reload();
                        }, 2000);
