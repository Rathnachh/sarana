<?php require_once('../_layout/head.php') ;?>
        ...
        <?php
            ...
            if(isset($_GET['ST']))
            {
                ...
                INNER JOIN tbl_floor AS fl ON fl.id = str.floor_id 
                INNER JOIN tbl_degree AS de ON de.id = st.degree_id
                WHERE fl.id=$floor_id AND st.feature=1 AND st.acept = 1";
        ...
        <div class="content shadow p-3">
            <div class="title d-flex">
                <a href="manage_floor.php"><i class="fa-solid fa-arrow-left mr-3" style="font-size:18px"></i></a>
                <h3>ឈ្នោះនិស្សិតដែលស្នាក់នៅ​ <span><?php echo $_GET['ROM'].' '.$_GET['BDN'] ?> ឆ្នាំ <?php echo date("Y")?></span></h3>
            </div>
                   ...
                   <tbody>
                        <?php
                            if($data_build->num_rows >0)
                            {
                                ...
                                {
                                    ?>
                                         <tr data-href="student_details.php?STU_ID=<?php echo $row['id']?>&link=view_st_floor.php?ST=<?php echo $_GET['ST'] ?>&BDN=<?php echo $_GET['BDN']?>&ROM=<?php echo $_GET['ROM']?>">
                                            <td><?php echo $n++; ?></td>
