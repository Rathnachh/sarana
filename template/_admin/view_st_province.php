<?php require_once('../_layout/head.php') ;?>
        ...
        <?php
            ...
            if(isset($_GET['ST']))
            {
                ...
                WHERE st.feature=1 AND st.acept=1 AND pr.province_id=$PROID";
        ...
        <div class="content shadow p-3">
            <div class="title d-flex">
                <a href="manage_province.php"><i class="fa-solid fa-arrow-left mr-3" style="font-size:18px"></i></a>
                <h3>ឈ្នោះនិស្សិតដែលស្នាក់នៅខេត្ត​ <span><?php echo $_GET['BDN'] ?> ឆ្នាំ <?php echo date("Y")?></span></h3>
            </div>
                   ...
                   <tbody>
                        <?php
                            if($data_build->num_rows >0)
                            {
                                ...
                                {
                                    ?>
                                         <tr data-href="student_details.php?STU_ID=<?php echo $row['id']?>&link=view_st_province.php?ST=<?php echo $_GET['ST'] ?>&BDN=<?php echo $_GET['BDN']?>">
                                            <td><?php echo $n++; ?></td>
